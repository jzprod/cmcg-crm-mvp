...
- A dedicated **Rapports** tab (`#reports`, admin only) shows a full per-agent report for the selected agent, respecting the reporting period: advertising KPIs (spend, messages, booked, visits, registered, cost per outcome), collected revenue, ROI and potential ROI, closing quality (show rate, close rate, closing score), and the agent's students with paid/remaining progress and due status. A "Imprimer / PDF" button prints just the report (print stylesheet hides chrome).
- `DELETE /api/students/:id` removes a student entirely along with their payments and timeline events (admin only; sales is blocked at the API gate). The student management view shows a "Supprimer l'étudiant" button (admin only) with a confirmation, for retracting a mistaken registration. Use status → "Annulé" instead when the record should be kept.
...
- `DELETE /api/students/:id` - permanently removes a student plus their payments and events (admin only).
