...
- Adds a language selector with right-to-left Arabic mode for the full platform.
...
CRM_SALES_USER=sales_agent_login
CRM_SALES_PASSWORD=sales_agent_private_password
CRM_SALES_AGENT=exact_agent_name_in_the_CRM
DB_HOST=localhost
DB_PORT=3306
...
Student, group, and payment tools are intentionally locked unless login variables are configured.

- Admin login: set `CRM_USER` and `CRM_PASSWORD`.
- Sales-agent login: set `CRM_SALES_USER`, `CRM_SALES_PASSWORD`, and `CRM_SALES_AGENT`.

`CRM_SALES_AGENT` must exactly match the sales agent name created inside the CRM, for example `Souad`. The sales login can open `/groups`, register students, and record payments, but it cannot see advertising spend, cost per lead, cost per registration, imports, backups, or reset tools.
